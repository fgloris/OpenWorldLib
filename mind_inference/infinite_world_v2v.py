...
def load_video_to_pil(video_path, max_frames = None):
...
video_input_path = "./data/test_case/test_video_case1/origin.mp4" 
...
# 3. 读取输入视频
input_frames = load_video_to_pil(video_input_path, 240)[:240]

# 4. 调用 v2v 接口进行续写
# 注意：这里传入的是 video_frames 列表，而不是单张 images
output_video = pipeline.v2v(
    video_frames=input_frames,
    prompt="A boardcast studio",
    interactions=["camera_r", "camera_r", "camera_r", "camera_r"],
    num_frames=240,      # 续写的帧数
    size=(384, 1024),   # 确保分辨率与模型要求一致
)

# 5. 保存结果
output_path = "infinite_world_v2v_demo.mp4"
save_uint8_video(pipeline.memory_module.all_frames, output_path, fps=24)
